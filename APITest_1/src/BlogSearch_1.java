...
public class BlogSearch_1 {
	private static final String CLIENT_ID = "	"; 
	private static final String CLIENT_SECRET = "	"; 
	private static final String API_URL = "https://openapi.naver.com/v1/search/blog?query="; //api주소는 네이버개발자센터 네이버블로그검색 java 예제에서 보고 가져옴
	...
	public static String searchBlog(String text) {
		...
		HttpURLConnection con = null; //HttpURLConnection는 HTTP 통신을 가능케 해주는 클래스
	        ...
	        try {
	            ...
	            while(true) { //받은 내용(str) 메세지(msg) 안에 넣기
	            ...
	            //파일 생성 여기부터
	            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");//파일 이름에 날짜넣기 위해 dateformat
	            String date = sdf.format(Calendar.getInstance().getTime()); //현재 날짜값 받기
	            text = URLDecoder.decode(text, "UTF-8");//위에 있던거랑 같아보이지만  URLDecoder이라 다르다. 수신측에서 받을때 씀
	            String fileName = date + "_" + text + ".html"; //.txt여도 상관없다.
